...
//-- HID releted
	...
	//! @brief		拡大タッチ操作が行われた
	//! @param		startdist	開始時の 2 点間の幅
	//! @param		curdist	現在の 2 点間の幅
	//! @param		cx		触れている幅
	//! @param		cy		触れている高さ
	//! @param		flag	タッチ状態フラグ
	...
	//! @brief		入力状態のチェック
	//! @note		ウィンドウから約 1 秒おきに、レイヤマネージャがユーザからの入力の状態を
...
public:
	...
	//! @param		x			描画矩形内の座標におけるマウスカーソルの x 位置
	//! @param		y			描画矩形内の座標におけるマウスカーソルの y 位置
	...
	//! @param		x			描画矩形内の座標における注視ポイントの x 位置
	//! @param		y			描画矩形内の座標における注視ポイントの y 位置
	...
	//! @brief		Window の iTJSDispatch2 インターフェースを取得する
...
//---- window interface 関連
	...
	//! @note		(TJS から) Window.drawDevice プロパティを設定した直後に呼ばれる。
...
//---- 描画位置・サイズ関連
	...
	//!				描画デバイスは、SetDestRectangle で指定された領域を、このメソッドで指定された矩形
...
//---- ユーザーインターフェース関連
	...
	//! @param		manager		レイヤマネージャ
	//! @param		x			プライマリレイヤ上の座標におけるマウスカーソルの x 位置
	//! @param		y			プライマリレイヤ上の座標におけるマウスカーソルの y 位置
	...
	//!				と異なることに注意。ウィンドウのマウスキャプチャは主に OS のウィンドウシステムの
	...
	//! @param		manager		レイヤマネージャ
	//! @param		layer		フォント情報の含まれるレイヤ
	//! @param		x			プライマリレイヤ上の座標における注視ポイントの x 位置
	//! @param		y			プライマリレイヤ上の座標における注視ポイントの y 位置
	//! @note		注視ポイントは通常キャレット位置のことで、そこに IME のコンポジット・ウィンドウが
	//!				表示されたり、ユーザ補助の拡大鏡がそこを拡大したりする。IME がコンポジットウィンドウを
...
//---- デバッグ支援
	...
	//! @param		color		16bpp の時 565 か 555 を指定
	...
	//! @param		w			要求する幅
	//! @param		h			要求する高さ
	//! @param		bpp			元々の Bit per pixels
	//! @param		color		16bpp の時 565 か 555 を指定
	virtual void TJS_INTF_METHOD RevertFromFullScreen( HWND window, tjs_uint w, tjs_uint h, tjs_uint bpp, tjs_uint color ) = 0;

	//! @brief		(Window->DrawDevice) VBlank 待ちを行う
	//! @param		in_vblank	待たなくても VBlank 内だったかどうかを返す ( !0 : 内、0: 外 )
	//! @param		delayed		1 フレーム遅延が発生したかどうかを返す ( !0 : 発生、0: 発生せず )
	//! @return		Wait 可不可 true : 可能、false : 不可
